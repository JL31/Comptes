...
class VerificationDonneesJSON(object):
    ...
    def __init__(self, contenu_du_fichier):
        ...
        self._liste_des_elements: Dict[str, List[str]] = {
            "prelevements": ["titre", "montant", "statut"],
    ...
    def verification_date_de_mise_a_jour(self):
        ...
        try:
            ...
            message_d_erreur = """
La date de mise-à-jour ne figure pas dans le fichier.
...
"""
    ...
    def verification_annee(self):
        ...
        try:
            ...
            message_d_erreur = """
L'année ne figure pas dans le fichier.
Veuillez le vérifier.
...
"""
    ...
    def recuperation_de_la_liste_des_mois(self):
        ...
        cles_a_exclure: List[str] = [
            "date_de_mise_a_jour",
            "annee"
        ]
        self._liste_des_mois: List[str] = [cle for cle in self._liste_des_cles if cle not in cles_a_exclure]
    ...
    def verification_des_mois(self):
        ...
        if len(self._liste_des_mois) < 12:
            liste_des_mois_manquants = [mois for mois in self._mois if mois not in self._liste_des_mois]
        else:
    ...
    def verification_elements(self, mois_en_cours, categorie):
        ...
        # Si l'indice en cours possède exactement le nombre d'éléments on vérifie que ce sont les bons, i.e. qu'ils
        # correspondent tous à l'un de ceux figurants dans la liste des éléments pour la catégorie associée
        for indice, element in enumerate(self._liste_des_elements[categorie]):
            if element not in self._liste_des_elements_indice_en_cours:
                message_d_erreur = "Il manque l'élément {} dans la catégorie {} pour le mois {}".format(element, categorie, mois_en_cours)
                raise KeyError(message_d_erreur)
    ...
    def verifications(self):
        ...
        # Vérification du contenu des catégories prélèvements, épargnes et dépenses
        categorie: str
        for categorie in ["prelevements", "epargnes", "depenses"]:
            self.verification_categorie(categorie)
